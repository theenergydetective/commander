 ...
 */
@Repository
public class HistoryMTUDayDAO extends SimpleAbstractDAO {

    private static final String INSERT = "INSERT INTO history_mtu_day2 " +
            "(virtualECC_id, " +
            "mtu_id, " +
            "startEpoch, " +
            "endEpoch, " +
            ...
            ":touMidPeak, " +
            ":touSuperPeak)";
    private static final String SAVE = "UPDATE history_mtu_day2" +
            "  SET " +
            "  energy =  :energy, " +
            "  cost =  :cost, " +
    ...
    private static final String FIND_BY_DATE_RANGE_QUERY = "select * from history_mtu_day2 where virtualECC_id=? and mtu_id = ? and startEpoch >= ? and startEpoch < ? order by startEpoch";
    @Autowired
    HazelcastService hazelcastService;
    @Autowired
    VirtualECCDAO virtualECCDAO;
    private RowMapper<HistoryMTUDay> rowMapper = new RowMapper<HistoryMTUDay>() {
        public HistoryMTUDay mapRow(ResultSet rs, int rowNum) throws SQLException {
            HistoryMTUDay dto = new HistoryMTUDay();
    ...
    private Map getNamedParameters(HistoryMTUDay dto) {
    ...
    private void updateCalendarKeys(HistoryMTUDay dto) {
    ...
    public HistoryMTUDay findOne(Long virtualECCId, Long mtuId, Long startEpoch) {

        HistoryMTUKey key = new HistoryMTUKey(virtualECCId, mtuId, startEpoch);
        try {
            HistoryMTUDay historyDay = hazelcastService.getHistoryMTUDayMap().get(key);
            if (historyDay == null) {
                LOGGER.debug("DATABASE HIT<<<<<<<<<<<<<<<<<<<<<<<<<<<");
                String query = "select *  from history_mtu_day2 where virtualECC_id=? and mtu_id=? and startEpoch=?";
                ...
                if (!PollingService.skipCache && PollingService.NUMBER_ENERGYPOST_THREADS != 0)
                    hazelcastService.getHistoryMTUDayMap().put(key, historyDay);
            }
            return historyDay;
    ...
    public void insert(HistoryMTUDay dto) {
        processBatch();
        try {
            ...
            if (!PollingService.skipCache && PollingService.NUMBER_ENERGYPOST_THREADS != 0) {
                updateCalendarKeys(dto);
                hazelcastService.getHistoryMTUDayMap().put(key, dto);
            }
//            getNamedParameterJdbcTemplate().update(INSERT, getNamedParameters(dto));
    ...
    public void save(HistoryMTUDay dto) {
        processBatch();
        try {
            HistoryMTUKey key = new HistoryMTUKey(dto.getVirtualECCId(), dto.getMtuId(), dto.getStartEpoch());
            if (!PollingService.skipCache && PollingService.NUMBER_ENERGYPOST_THREADS != 0) {
    ...
    public List<HistoryMTUDay> findByDateRange(Long virtualECCId, long mtu_id, long startEpoch, long endEpoch) {
    ...
    Cache<HistoryMTUKey, HistoryMTUDay> batch = CacheBuilder.newBuilder().concurrencyLevel(1).expireAfterAccess(30, TimeUnit.MINUTES).maximumSize(BATCH_SIZE).initialCapacity(BATCH_SIZE).build();

    synchronized void queueForBatch(HistoryMTUKey key, HistoryKey.BatchState batchState, HistoryMTUDay dto){
    ...
    public synchronized void processBatch(){
        ...
        List<HistoryMTUDay> insertRecords = new ArrayList<>(BATCH_SIZE);
        List<HistoryMTUDay> updateRecords = new ArrayList<>(BATCH_SIZE);
    ...
    private void batchUpdate(String query, List<HistoryMTUDay> batchList) {
    ...
    public void deleteByVirtualEcc(Long virtualECCId, Long mtuId) {
        getJdbcTemplate().update("DELETE FROM history_mtu_day2 where virtualECC_id=? and mtu_id=? ", new Object[]{virtualECCId, mtuId});
    }
}
